...
namespace Watsonia.Data.Tests
{
	...
	public class DocumentationTests
	{
		...
		public void TestUndoRedo()
		{
			...
			// Save and re-load the author and book to clear the changes
			db.Save(author);
			author = db.Load<Author>(((IDynamicProxy)author).PrimaryKeyValue);
			book1 = author.Books[0];

			// There shouldn't be any changes
			Assert.AreEqual(0, ((IDynamicProxy)author).BuildUndoQueue().Count, "Author shouldn't have changes");
			Assert.AreEqual(0, ((IDynamicProxy)book1).BuildUndoQueue().Count, "Book shouldn't have changes");
			
			// Oops!  Have to fix some mistakes
			author.LastName = "Dickens";
			book1.Title = "A Tale of Two Cities";

			// There should be two changes for the author as it includes the child changes
			Assert.AreEqual(2, ((IDynamicProxy)author).BuildUndoQueue().Count, "Author should have 2 changes");
			Assert.AreEqual(1, ((IDynamicProxy)book1).BuildUndoQueue().Count, "Book should have 1 change");
			Assert.AreEqual("Title", ((IDynamicProxy)author).BuildUndoQueue()[((IDynamicProxy)author).BuildUndoQueue().Count - 1].PropertyName, "Title should be the last change");
			
			// Let's add another book
			Book book2 = db.Create<Book>();
			book2.Title = "Paxton's Bloomers";
			author.Books.Add(book2);

			// Check the author's changes again
			Assert.AreEqual(4, ((IDynamicProxy)author).BuildUndoQueue().Count, "Author should have 4 changes");

			// Oops!  That's not by Dickens
			author.Undo();
			Assert.AreEqual(2, ((IDynamicProxy)author).BuildUndoQueue().Count, "Author should have 2 changes again");
			Assert.AreEqual(1, ((IDynamicProxy)author).BuildRedoQueue().Count, "Author should have 1 change to redo");
			Assert.AreEqual(1, author.Books.Count, "Author should have 1 book");

			// Let's save the author and check there are no changes
			db.Save(author);
			Assert.IsFalse(author.HasChanges, "Author shouldn't have changes");
			Assert.IsFalse(book1.HasChanges, "Book shouldn't have changes");

			// Eh
			author.Redo();
			Assert.AreEqual(4, ((IDynamicProxy)author).BuildUndoQueue().Count, "Author should have 4 changes again");
			Assert.AreEqual(2, author.Books.Count, "Author should have 2 books");

			// "Accidentally" change the title of the first book
			book1.Title = "Jonah's Account of the Whale";

			// Both of those changes are silly, let's undo them again
			author.Undo();
			author.Undo();
			Assert.AreEqual(2, ((IDynamicProxy)author).BuildUndoQueue().Count, "Author should have 2 changes yet again");
			Assert.AreEqual(2, ((IDynamicProxy)author).BuildRedoQueue().Count, "Author should have 2 changes to redo");
			Assert.AreEqual(1, author.Books.Count, "Author should have 1 book");
			Assert.AreEqual("A Tale of Two Cities", author.Books[0].Title, "The first book should have the correct title");

			// The author shouldn't have changes after all that undoing
			Assert.IsFalse(author.HasChanges, "Author shouldn't have changes after undoing");
			Assert.IsFalse(book1.HasChanges, "Book shouldn't have changes after undoing");
		}
	}
}