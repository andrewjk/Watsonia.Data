...
// NOTE: This is a copy of IQToolkit's ExecutionBuilder with ExecutionBuilder replaced with DynamicExecutionBuilder and
// QueryCommand replaced with QueryBlock
namespace Watsonia.Data.Linq
{
	/// <summary>
	/// Builds an execution plan for a query expression
	/// </summary>
	internal class DynamicExecutionBuilder : DbExpressionVisitor
	{
		QueryPolicy policy;
		...
		private DynamicExecutionBuilder(QueryLinguist linguist, QueryPolicy policy, Expression executor)
		{
		...
		public static Expression Build(QueryLinguist linguist, QueryPolicy policy, Expression expression, Expression provider)
		{
			...
			var builder = new DynamicExecutionBuilder(linguist, policy, executor);
		...
		private static Expression MakeSequence(IList<Expression> expressions)
		{
			...
			return Expression.Convert(Expression.Call(typeof(DynamicExecutionBuilder), "Sequence", null, Expression.NewArrayInit(typeof(object), expressions)), last.Type);
		}
		...
		{
			return Expression.Call(typeof(DynamicExecutionBuilder), "Assign", new Type[] { variable.Type }, variable, value);
		}
		...
		{
			var eb = new DynamicExecutionBuilder(this.linguist, this.policy, this.executor);
			eb.scope = this.scope;
		...
		private Expression ExecuteProjection(ProjectionExpression projection, bool okayToDefer)
		{
			...
			QueryBlock command = new QueryBlock(commandText, projection.Select, namedValues.Select(v => new QueryParameter(v.Name, v.Type, v.QueryType)));
		...
		private Expression ExecuteProjection(ProjectionExpression projection, bool okayToDefer, QueryBlock command, Expression[] values)
		{
		...
		protected virtual Expression BuildExecuteBatch(BatchExpression batch)
		{
			...
			QueryBlock command = new QueryBlock(commandText, operation, namedValues.Select(v => new QueryParameter(v.Name, v.Type, v.QueryType)));
			Expression[] values = namedValues.Select(v => Expression.Convert(this.Visit(v.Value), typeof(object))).ToArray();
			...
			if (projection != null)
			{
				...
				var entity = EntityFinder.Find(projection.Projector);
				command = new QueryBlock(command.CommandText, command.Expression, command.Parameters);
		...
		protected virtual Expression BuildExecuteCommand(CommandExpression command)
		{
			...
			QueryBlock qc = new QueryBlock(commandText, expression, namedValues.Select(v => new QueryParameter(v.Name, v.Type, v.QueryType)));
			Expression[] values = namedValues.Select(v => Expression.Convert(this.Visit(v.Value), typeof(object))).ToArray();
