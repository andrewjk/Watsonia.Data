...
namespace Watsonia.Data.Query
{
	...
	public class ExecutionBuilder : DbExpressionVisitor
	{
		...
		private ExecutionBuilder(Expression executor)
		{
			this.executor = executor;
		}

		public static Expression Build(Expression expression, Expression provider)
		{
			var executor = Expression.Parameter(typeof(QueryExecutor), "executor");
			var builder = new ExecutionBuilder(executor);
			builder.variables.Add(executor);
			builder.initializers.Add(Expression.Call(Expression.Convert(provider, typeof(QueryProvider)), "CreateExecutor", null, null));
		...
		{
			var eb = new ExecutionBuilder(this.executor);
			eb.scope = this.scope;
		...
		protected override Expression VisitClientJoin(ClientJoinExpression join)
		{
			...
			ProjectionExpression newProjection = new ProjectionExpression(join.Projection.SelectExpression, constructKVPair);
		...
		private Expression ExecuteProjection(ProjectionExpression projection, bool okayToDefer)
		{
			...
			ReadOnlyCollection<NamedValueExpression> namedValues = NamedValueGatherer.Gather(projection.SelectExpression);
			QueryCommand command = new QueryCommand(projection.SelectExpression, namedValues.Select(v => new QueryCommandParameter(v.Name, v.Type, v.ValueType)));
		...
		private Expression ExecuteProjection(ProjectionExpression projection, bool okayToDefer, QueryCommand command, Expression[] values)
		{
			...
			this.scope = new Scope(this.scope, reader, projection.SelectExpression.Alias, projection.SelectExpression.Columns);
		...
		protected virtual Expression BuildExecuteBatch(BatchExpression batch)
		{
			...
			QueryCommand command = new QueryCommand(operation, namedValues.Select(v => new QueryCommandParameter(v.Name, v.Type, v.ValueType)));
			Expression[] values = namedValues.Select(v => Expression.Convert(this.Visit(v.Value), typeof(object))).ToArray();

			Expression paramSets = Expression.Call(typeof(Enumerable), "SelectExpression", new Type[] { batch.Operation.Parameters[1].Type, typeof(object[]) },
				batch.Input,
			...
			if (projection != null)
			{
				...
				var entity = EntityFinder.Find(projection.Projector);
				command = new QueryCommand(command.Expression, command.Parameters);
		...
		{
			if (IsRowsAffectedExpressions(func))
			{
		...
		private bool IsRowsAffectedExpressions(Expression expression)
		{
		...
		protected override Expression VisitExists(ExistsExpression exists)
		{
			...
			var newSelect = exists.SelectExpression.SetColumns(
				new[] { new ColumnDeclaration("value", new AggregateExpression(typeof(int), "Count", null, false), typeof(int)) }
				);
		...
		protected virtual Expression BuildExecuteCommand(CommandExpression command)
		{
			...
			QueryCommand qc = new QueryCommand(expression, namedValues.Select(v => new QueryCommandParameter(v.Name, v.Type, v.ValueType)));
			Expression[] values = namedValues.Select(v => Expression.Convert(this.Visit(v.Value), typeof(object))).ToArray();
		...
		class OuterParameterizer : DbExpressionVisitor
		{
			...
			{
				SelectExpression select = (SelectExpression)this.Visit(proj.SelectExpression);
			...
			protected override Expression VisitColumn(ColumnExpression column)
			{
				if (column.Alias == this.outerAlias)
				{
					...
					{
						nv = new NamedValueExpression("n" + (iParam++), column.Type, column);
