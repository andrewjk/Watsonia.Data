	...
	internal sealed class Parameterizer : DbExpressionVisitor
	{
		...
		protected override Expression VisitBinary(BinaryExpression b)
		{
			 ...
			 && right.NodeType == (ExpressionType)DbExpressionType.Column)
			{
				...
				left = new NamedValueExpression(nv.Name, c.Type, nv.Value);
			}
			 ...
			 && b.Left.NodeType == (ExpressionType)DbExpressionType.Column)
			{
				...
				right = new NamedValueExpression(nv.Name, c.Type, nv.Value);
			}
		...
		protected override ColumnAssignment VisitColumnAssignment(ColumnAssignment ca)
		{
			...
			if (nv != null)
			{
				expression = new NamedValueExpression(nv.Name, ca.Column.Type, nv.Value);
			}
		...
		protected override Expression VisitConstant(ConstantExpression c)
		{
			if (c.Value != null && !IsNumeric(c.Value.GetType()))
			{
				...
				if (!this.map.TryGetValue(tv, out nv))
				{
					...
					nv = new NamedValueExpression(name, c.Type, c);
					this.map.Add(tv, nv);
				}
				return nv;
			}
		...
		private Expression GetNamedValue(Expression e)
		{
			...
			{
				string name = (iParam++).ToString();
				nv = new NamedValueExpression(name, e.Type, e);
				this.pmap.Add(he, nv);
			}
			return nv;
		}
