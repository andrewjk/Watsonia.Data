...
namespace Watsonia.Data.Query.Translation
{
    ...
    internal sealed class CrossJoinIsolator : DbExpressionVisitor
    {
        ...
        protected override Expression VisitJoin(JoinExpression join)
        {
			...
			if (this.lastJoin != null && (join.Join == ExpressionJoinType.CrossJoin) != (this.lastJoin == ExpressionJoinType.CrossJoin))
            {
        ...
        private bool IsCrossJoin(Expression expression)
        {
            ...
            if (jex != null)
            {
				return jex.Join == ExpressionJoinType.CrossJoin;
            }
            return false;
        }
        ...
        private Expression MakeSubquery(Expression expression)
        {
            ...
            foreach (var ta in aliases) 
            {
                foreach (var col in this.columns[ta])
                {
                    ...
                    var decl = new ColumnDeclaration(name, col, col.Type);
                    decls.Add(decl);
                    var newCol = new ColumnExpression(col.Type, newAlias, name);
