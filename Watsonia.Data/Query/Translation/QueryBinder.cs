...
namespace Watsonia.Data.Query.Translation
{
	...
	public class QueryBinder : DbExpressionVisitor
	{
		...
		protected override Expression Visit(Expression exp)
		{
			...
			if (result != null)
			{
				...
				if (projection != null && projection.Aggregator == null && !expectedType.IsAssignableFrom(projection.Type))
				{
					...
					if (aggregator != null)
					{
						return new ProjectionExpression(projection.SelectExpression, projection.Projector, aggregator);
					}
				}
			}
		...
		private Expression BindReverse(Expression source)
		{
			...
			ProjectedColumns pc = this.ProjectColumns(projection.Projector, alias, projection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, projection.SelectExpression, null).SetReverse(true),
				pc.Projector
				);
		}
		...
		private Expression BindSelect(Type resultType, Expression source, LambdaExpression selector)
		{
			...
			// TODO: Search through the mapped parameters for one that matches except that it has the dynamic proxy type?

			this.map[selector.Parameters[0]] = projection.Projector;
			Expression expression = this.Visit(selector.Body);
			var alias = this.GetNextAlias();
			ProjectedColumns pc = this.ProjectColumns(expression, alias, projection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, projection.SelectExpression, null),
				pc.Projector
				);
		}
		...
		protected virtual Expression BindSelectMany(Type resultType, Expression source, LambdaExpression collectionSelector, LambdaExpression resultSelector)
		{
			...
			bool isTable = collectionProjection.SelectExpression.From is TableExpression;
			ExpressionJoinType joinType = isTable ? ExpressionJoinType.CrossJoin : defaultIfEmpty ? ExpressionJoinType.OuterApply : ExpressionJoinType.CrossApply;
			if (joinType == ExpressionJoinType.OuterApply)
			{
				collectionProjection = OuterJoinTester.AddOuterJoinTest(collectionProjection);
			}
			JoinExpression join = new JoinExpression(joinType, projection.SelectExpression, collectionProjection.SelectExpression, null);

			var alias = this.GetNextAlias();
			ProjectedColumns pc;
			if (resultSelector == null)
			{
				pc = this.ProjectColumns(collectionProjection.Projector, alias, projection.SelectExpression.Alias, collectionProjection.SelectExpression.Alias);
			}
			else
			{
				...
				pc = this.ProjectColumns(result, alias, projection.SelectExpression.Alias, collectionProjection.SelectExpression.Alias);
			}
			return new ProjectionExpression(
		...
		protected virtual Expression BindJoin(Type resultType, Expression outerSource, Expression innerSource, LambdaExpression outerKey, LambdaExpression innerKey, LambdaExpression resultSelector)
		{
			...
			JoinExpression join = new JoinExpression(ExpressionJoinType.InnerJoin, outerProjection.SelectExpression, innerProjection.SelectExpression, outerKeyExpr.Equal(innerKeyExpr));
			var alias = this.GetNextAlias();
			ProjectedColumns pc = this.ProjectColumns(resultExpr, alias, outerProjection.SelectExpression.Alias, innerProjection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, join, null),
				pc.Projector
				);
		...
		protected virtual Expression BindIntersect(Expression outerSource, Expression innerSource, bool negate)
		{
			...
			Expression exists = new ExistsExpression(
				new SelectExpression(new TableAlias(), null, innerProjection.SelectExpression, innerProjection.Projector.Equal(outerProjection.Projector))
				);
			if (negate)
				exists = Expression.Not(exists);
			var alias = this.GetNextAlias();
			ProjectedColumns pc = this.ProjectColumns(outerProjection.Projector, alias, outerProjection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, outerProjection.SelectExpression, exists),
		...
		protected virtual Expression BindGroupJoin(MethodInfo groupJoinMethod, Expression outerSource, Expression innerSource, LambdaExpression outerKey, LambdaExpression innerKey, LambdaExpression resultSelector)
		{
			...
			var alias = this.GetNextAlias();
			ProjectedColumns pc = this.ProjectColumns(resultExpr, alias, outerProjection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, outerProjection.SelectExpression, null),
				pc.Projector
				);
		}
		...
		protected virtual Expression BindOrderBy(Type resultType, Expression source, LambdaExpression orderSelector, OrderType orderType)
		{
			...
			ProjectedColumns pc = this.ProjectColumns(projection.Projector, alias, projection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, projection.SelectExpression, null, orderings.AsReadOnly(), null),
				pc.Projector
				);
		}
		...
		protected virtual Expression BindGroupBy(Expression source, LambdaExpression keySelector, LambdaExpression elementSelector, LambdaExpression resultSelector)
		{
			...
			ProjectedColumns keyProjection = this.ProjectColumns(keyExpr, projection.SelectExpression.Alias, projection.SelectExpression.Alias);
			...
			ProjectedColumns subqueryKeyPC = this.ProjectColumns(subqueryKey, subqueryBasis.SelectExpression.Alias, subqueryBasis.SelectExpression.Alias);
			...
			ProjectedColumns elementPC = this.ProjectColumns(subqueryElemExpr, elementAlias, subqueryBasis.SelectExpression.Alias);
			ProjectionExpression elementSubquery =
				new ProjectionExpression(
					new SelectExpression(elementAlias, elementPC.Columns, subqueryBasis.SelectExpression, subqueryCorrelation),
					elementPC.Projector
					);
			...
			ProjectedColumns pc = this.ProjectColumns(resultExpr, alias, projection.SelectExpression.Alias);
			...
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, projection.SelectExpression, null, null, groupExprs),
				pc.Projector
				);
		}
		...
		private Expression BindAggregate(Expression source, string aggName, Type returnType, LambdaExpression argument, bool isRoot)
		{
			...
			bool hasPredicateArg = (aggName == "Count" || aggName == "LongCount");
			...
			var alias = this.GetNextAlias();
			var pc = this.ProjectColumns(projection.Projector, alias, projection.SelectExpression.Alias);
			Expression aggExpr = new AggregateExpression(returnType, aggName, argExpr, isDistinct);
			SelectExpression select = new SelectExpression(alias, new ColumnDeclaration[] { new ColumnDeclaration("", aggExpr, returnType) }, projection.SelectExpression, null);
		...
		{
			ProjectionExpression projection = this.VisitSequence(source);
			SelectExpression select = projection.SelectExpression;
			var alias = this.GetNextAlias();
			ProjectedColumns pc = this.ProjectColumns(projection.Projector, alias, projection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, projection.SelectExpression, null, null, null, true, null, null, false),
				pc.Projector
				);
		}
		...
		private Expression BindTake(Expression source, Expression take)
		{
			...
			ProjectedColumns pc = this.ProjectColumns(projection.Projector, alias, projection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, projection.SelectExpression, null, null, null, false, null, take, false),
				pc.Projector
				);
		}
		...
		private Expression BindSkip(Expression source, Expression skip)
		{
			...
			ProjectedColumns pc = this.ProjectColumns(projection.Projector, alias, projection.SelectExpression.Alias);
			return new ProjectionExpression(
				new SelectExpression(alias, pc.Columns, projection.SelectExpression, null, null, null, false, skip, null, false),
				pc.Projector
				);
		}
		...
		private Expression BindFirst(Expression source, LambdaExpression predicate, string kind, bool isRoot)
		{
			...
			if (take != null || where != null)
			{
				...
				projection = new ProjectionExpression(
					new SelectExpression(alias, pc.Columns, projection.SelectExpression, where, null, null, false, null, take, isLast),
					pc.Projector
					);
			}
			if (isRoot)
			{
				...
				return new ProjectionExpression(projection.SelectExpression, projection.Projector, gator);
			}
			return projection;
		}
		...
		private Expression BindAnyAll(Expression source, MethodInfo method, LambdaExpression predicate, bool isRoot)
		{
			...
			else
			{
				...
				ProjectionExpression projection = this.VisitSequence(source);
				Expression result = new ExistsExpression(projection.SelectExpression);
				if (isAll)
				{
				...
				if (isRoot)
				{
					...
					//	var newSelect = projection.SelectExpression.SetColumns(
		...
		private Expression BindContains(Expression source, Expression match, bool isRoot)
		{
			...
			else
			{
				ProjectionExpression projection = this.VisitSequence(source);
				match = this.Visit(match);
				Expression result = new InExpression(match, projection.SelectExpression);
				if (isRoot)
				{
		...
		private Expression GetSingletonSequence(Expression expr, string aggregator)
		{
			...
			var alias = this.GetNextAlias();
			SelectExpression select = new SelectExpression(alias, new[] { new ColumnDeclaration("value", expr, expr.Type) }, null, null);
		...
		protected override Expression VisitConstant(ConstantExpression c)
		{
			if (this.IsQuery(c))
			{
				...
				if (t != null)
				{
					MappingEntity entity = this.mapper.Mapping.GetEntity(t.ElementType, t.TableId);
		...
		public static Expression BindMember(Expression source, MemberInfo member)
		{
			switch (source.NodeType)
			{
				...
				case (ExpressionType)DbExpressionType.Projection:
				{
					...
					return new ProjectionExpression(proj.SelectExpression, newProjector, Aggregator.GetAggregator(mt, typeof(IEnumerable<>).MakeGenericType(mt)));
				}
