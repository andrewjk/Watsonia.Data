...
namespace Watsonia.Data.Query.Translation
{
	...
	public class ClientJoinedProjectionRewriter : DbExpressionVisitor
	{
		...
		private ClientJoinedProjectionRewriter()
		{
		}
			...
			try
			{
				if (!this.isTopLevel)
				{
					if (this.CanJoinOnClient(this.currentSelect))
					{
						...
						SelectExpression newInnerSelect = (SelectExpression)ColumnMapper.Map(proj.SelectExpression, newOuterSelect.Alias, save.Alias);
						// add outer-join test
						ProjectionExpression newInnerProjection = OuterJoinTester.AddOuterJoinTest(new ProjectionExpression(newInnerSelect, proj.Projector));
						newInnerSelect = newInnerProjection.SelectExpression;
						...
						JoinExpression join = new JoinExpression(ExpressionJoinType.OuterApply, newOuterSelect, newInnerSelect, null);
