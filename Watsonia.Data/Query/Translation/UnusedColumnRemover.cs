    ...
    internal sealed class UnusedColumnRemover : DbExpressionVisitor
    {
        ...
        protected override Expression VisitSubquery(SubqueryExpression subquery) 
        {
            if ((subquery.NodeType == (ExpressionType)DbExpressionType.Scalar ||
                ...
                subquery.SelectExpression != null) 
            {
                System.Diagnostics.Debug.Assert(subquery.SelectExpression.Columns.Count == 1);
        ...
        protected override Expression VisitSelect(SelectExpression select)
        {
            ...
            for (int i = 0, n = select.Columns.Count; i < n; i++)
            {
                ...
                if (wasRetained || select.IsDistinct || IsColumnUsed(select.Alias, decl.Name))
                {
                    ...
                    {
                        decl = new ColumnDeclaration(decl.Name, expr, decl.ColumnType);
                    }
                }
                else
                {
        ...
        protected override Expression VisitProjection(ProjectionExpression projection)
        {
            ...
            SelectExpression select = (SelectExpression)this.Visit(projection.SelectExpression);
        ...
        {
			if (join.Join == ExpressionJoinType.SingletonLeftOuter)
            {
