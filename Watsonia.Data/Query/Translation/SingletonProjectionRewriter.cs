...
namespace Watsonia.Data.Query.Translation
{
	...
	internal sealed class SingletonProjectionRewriter : DbExpressionVisitor
	{
		...
		protected override Expression VisitProjection(ProjectionExpression proj)
		{
			...
			{
				isTopLevel = false;
				this.currentSelect = proj.SelectExpression;
				Expression projector = this.Visit(proj.Projector);
				if (projector != proj.Projector || this.currentSelect != proj.SelectExpression)
				{
			...
			if (proj.IsSingleton && this.CanJoinOnServer(this.currentSelect))
			{
				...
				SelectExpression source = (SelectExpression)ColumnMapper.Map(proj.SelectExpression, newAlias, this.currentSelect.Alias);

				// add outer-join test
				ProjectionExpression pex = OuterJoinTester.AddOuterJoinTest(new ProjectionExpression(source, proj.Projector));

				var pc = ColumnProjector.ProjectColumns(pex.Projector, this.currentSelect.Columns, this.currentSelect.Alias, newAlias, proj.SelectExpression.Alias);

				JoinExpression join = new JoinExpression(ExpressionJoinType.OuterApply, this.currentSelect.From, pex.SelectExpression, null);
