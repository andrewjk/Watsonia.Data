	...
	internal class StatementCreator : DbExpressionVisitor
	{
		...
		protected override Expression Visit(Expression exp)
		{
			...
			switch (exp.NodeType)
			{
				...
				case (ExpressionType)DbExpressionType.SelectExpression:
			...
			else if (m.Method.DeclaringType == typeof(Math))
			{
				switch (m.Method.Name)
				{
					...
					case "Atan2":
					{
						...
						this.Visit(m.Arguments[0]);
						newFunction.Argument = this.Stack.Pop();
						newFunction.Function = (TrigFunction)Enum.Parse(typeof(TrigFunction), m.Method.Name);
						this.Stack.Push(newFunction);
						return m;
					}
				}
		...
		protected override Expression VisitBinary(BinaryExpression b)
		{
			...
			switch (b.NodeType)
			{
				...
				case ExpressionType.GreaterThanOrEqual:
				{
					...
					this.VisitValue(left);
					newCondition.Field = this.Stack.Pop();
					newCondition.Operator = GetSqlOperator(b);
					this.VisitValue(right);
					newCondition.Value = this.Stack.Pop();
					this.Stack.Push(newCondition);
					break;
				...
				case ExpressionType.Coalesce:
				{
					...
					this.VisitValue(left);
					newCoalesce.Arguments.Add((SourceExpression)this.Stack.Pop());
		...
		private SqlOperator GetSqlOperator(BinaryExpression binary)
		{
			switch (binary.NodeType)
			{
				...
				default:
				{
					throw new InvalidOperationException("Invalid NodeType: " + binary.NodeType);
				}
			}
		}
		...
		protected virtual Expression VisitPredicate(Expression expr)
		{
			...
			if (!IsPredicate(expr))
			{
				...
				newCondition.Value = new ConstantPart(false);
				this.Stack.Push(newCondition);
			}
			return expr;
		}
		...
		protected override Expression VisitProjection(ProjectionExpression proj)
		{
			...
			{
				this.Visit(proj.SelectExpression);
			}
			else
			{
		...
		protected override Expression VisitSelect(SelectExpression select)
		{
			...
			if (select.Skip != null)
			{
				newSelect.SelectStartIndex = (int)((ConstantExpression)select.Skip).Value;
			}
			...
			if (select.Alias != null && _aliases.ContainsKey(select.Alias))
			{
		...
		protected override Expression VisitSource(Expression source)
		{
			...
			switch ((DbExpressionType)source.NodeType)
			{
				...
				default:
				{
					throw new InvalidOperationException("SelectExpression source is not valid type");
				}
			}
		...
		protected override Expression VisitJoin(JoinExpression join)
		{
			...
			if (join.Condition != null)
			{
				...
				while (this.Stack.Count > 0 && (this.Stack.Peek() is ConditionExpression || this.Stack.Peek() is UnaryOperation))
				{
					...
					{
						// Insert the conditions in reverse order from the stack
						newJoin.Conditions.Insert(0, (ConditionExpression)part);
					}
					else if (part is UnaryOperation)
					{
						UnaryOperation unary = (UnaryOperation)part;
						newJoin.Conditions.Insert(0, (ConditionExpression)unary.Expression);
						if (unary.Operator == UnaryOperator.Not)
						{
							newJoin.Conditions[0].Not = true;
						}
					}
				}
			}
		...
		private Watsonia.Data.JoinType GetJoinTypeFromExpression(JoinExpression join)
		{
			switch (join.Join)
			{
				case ExpressionJoinType.CrossApply:
				{
					return Watsonia.Data.JoinType.CrossApply;
				}
				case ExpressionJoinType.CrossJoin:
				{
					return Watsonia.Data.JoinType.Cross;
				}
				case ExpressionJoinType.InnerJoin:
				{
					return Watsonia.Data.JoinType.Inner;
				}
				case ExpressionJoinType.LeftOuter:
				{
					return Watsonia.Data.JoinType.Left;
				}
				case ExpressionJoinType.OuterApply:
				{
					throw new NotSupportedException();
				}
				case ExpressionJoinType.SingletonLeftOuter:
				{
					throw new NotSupportedException();
				}
				default:
		...
		protected override Expression VisitIsNull(IsNullExpression isnull)
		{
			...
			newCondition.Value = new ConstantPart(null);
			this.Stack.Push(newCondition);
			return isnull;
		}
		...
		protected override Expression VisitScalar(ScalarExpression subquery)
		{
			...
			this.VisitSelect(subquery.SelectExpression);
			field.Select = (Select)this.Stack.Pop();
		...
		protected override Expression VisitExists(ExistsExpression exists)
		{
			...
			this.Visit(exists.SelectExpression);
		...
		protected override Expression VisitIn(InExpression @in)
		{
			...
			if (@in.SelectExpression != null)
			{
				this.Visit(@in.SelectExpression);
				newCondition.Value = this.Stack.Pop();
			}
			else if (@in.Values != null)
			{
				List<StatementPart> values = new List<StatementPart>();
				foreach (Expression exp in @in.Values)
				{
					this.Visit(exp);
					values.Add(this.Stack.Pop());
				}
				newCondition.Value = new ConstantPart(values);
			}
