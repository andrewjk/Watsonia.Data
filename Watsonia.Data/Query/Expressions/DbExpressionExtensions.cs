...
namespace Watsonia.Data.Query.Expressions
{
	internal static class DbExpressionExtensions
	{
		...
		{
			string colName = proj.SelectExpression.Columns.GetAvailableColumnName("Test");
			SelectExpression newSource = proj.SelectExpression.AddColumn(new ColumnDeclaration(colName, expression, expression.Type));
		...
		public static SelectExpression AddRedundantSelect(this SelectExpression sel, TableAlias newAlias)
		{
			var newColumns =
				...
				let qt = (d.Expression is ColumnExpression) ? ((ColumnExpression)d.Expression).Type : d.Expression.Type
				select new ColumnDeclaration(d.Name, new ColumnExpression(d.Expression.Type, newAlias, d.Name), qt);
