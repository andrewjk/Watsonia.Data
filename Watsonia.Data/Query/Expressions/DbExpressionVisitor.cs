...
namespace Watsonia.Data.Query.Expressions
{
    ...
    internal abstract class DbExpressionVisitor : Watsonia.Data.Query.ExpressionVisitor
    {
        ...
        protected JoinExpression UpdateJoin(JoinExpression join, ExpressionJoinType joinType, Expression left, Expression right, Expression condition)
        {
        ...
        {
            var select = (SelectExpression)this.Visit(scalar.SelectExpression);
        ...
        {
            if (select != scalar.SelectExpression)
            {
        ...
        {
            var select = (SelectExpression)this.Visit(exists.SelectExpression);
        ...
        {
            if (select != exists.SelectExpression)
            {
        ...
        protected virtual Expression VisitIn(InExpression @in)
        {
            ...
            var select = (SelectExpression)this.Visit(@in.SelectExpression);
        ...
        {
            if (expression != @in.Expression || select != @in.SelectExpression || values != @in.Values)
            {
                if (select != null)
                {
        ...
        {
            var select = (SelectExpression)this.Visit(proj.SelectExpression);
        ...
        {
            if (select != proj.SelectExpression || projector != proj.Projector || aggregator != proj.Aggregator)
            {
        ...
        protected virtual ReadOnlyCollection<ColumnDeclaration> VisitColumnDeclarations(ReadOnlyCollection<ColumnDeclaration> columns)
        {
            ...
            for (int i = 0, n = columns.Count; i < n; i++)
            {
                ...
                if (alternate != null)
                {
                    alternate.Add(new ColumnDeclaration(column.Name, e, column.ColumnType));
                }
            }
            if (alternate != null)
            {
