...
namespace Watsonia.Data.SQLite
{
	public class SqlCommandBuilder
	{
		...
		public void VisitStatement(Statement statement, DatabaseConfiguration configuration)
		{
			switch (statement.PartType)
			{
				...
				case StatementPartType.GenericInsert:
				{
					var messageType = statement.GetType();
					var method = statement.GetType().GetMethod("CreateStatement");
					var select = (InsertStatement)method.Invoke(statement, new object[] { configuration });
					VisitInsert(select);
					break;
				}
				...
				case StatementPartType.GenericUpdate:
				{
					var messageType = statement.GetType();
					var method = statement.GetType().GetMethod("CreateStatement");
					var select = (UpdateStatement)method.Invoke(statement, new object[] { configuration });
					VisitUpdate(select);
					break;
				}
				...
				case StatementPartType.GenericDelete:
				{
					var messageType = statement.GetType();
					var method = statement.GetType().GetMethod("CreateStatement");
					var select = (DeleteStatement)method.Invoke(statement, new object[] { configuration });
					VisitDelete(select);
					break;
				}
				default:
		...
		{
			var index = this.ParameterValues.IndexOf(parameter.Value);
			if (index == -1)
			{
